...
Design and rationale: `docs/recall/design.md`.
...
the index (about 90 s in this repository); every later query is ~1.5 s. So when
ingest is slow, a moved HEAD is usually why.

```sh
entire graph index --repo .     # warm the cache once, after your last commit
...
Moving HEAD between the two steps — a commit, amend, rebase or checkout — makes
the next ingest pay the full re-index again; re-run `entire graph index` when
that happens. `entire recall ingest --no-graph` skips the graph entirely: fast,
but the isolation check can never fire.
